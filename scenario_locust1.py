...
class HttpTaskSet(TaskSet):
    c_data = {
        "cno": "1586313249694323",
        "shop_id": 1905736354,
        "cashier_id": "1180940478",
        "money": "100",
        "award_money2": 10,
        "reward_money": "100",
        "is_diy": 'true',
    ...
    @task
    def i_charge(self):
        biz_id = '{}{}'.format(int(time.time()),self.genrandomstr(5))
        #储值数据和头信息
        self.c_data['biz_id'] = biz_id
    ...
    def i_commit(self):
        ...
        while not self.commit_queue.empty():
            biz_id = self.commit_queue.get()
            self.commit_data['biz_id'] = biz_id
    ...
    def i_cancel(self):
        ...
        while not self.cancel_queue.empty():
            biz_id = self.cancel_queue.get()
            self.cancel_data['biz_id'] = biz_id
